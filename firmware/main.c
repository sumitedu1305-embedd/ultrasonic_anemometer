...
int main(void)
{
	...
	GPIO_Set(PWR_ON_GPIO_Port,PWR_ON_Pin,0); // mux always on

	GPIO_Set(PWR_DRV0_GPIO_Port,PWR_DRV0_Pin,1);    // south west driver on
	GPIO_Set(PWR_DRV1_GPIO_Port,PWR_DRV1_Pin,1);    // north east driver ons
	
	delay_ms(1);    // delay require for charge pump capacitor
	...
	while(1)
	{
        ...
        while (DMA_CCR(DMA1, DMA_CHANNEL_1) & (1 << 0)){}
        
        DMA_CMAR(DMA1, DMA_CHANNEL_1)       =  (uint32_t)(adc_buffer1 + 1); // memory = buffer
        DMA_IFCR(DMA1)                      =  0xF;
        ...
        DMA_CMAR(DMA1, DMA_CHANNEL_1)       = (uint32_t)(adc_buffer2 + 1); // memory = buffer
        DMA_IFCR(DMA1)                      = 0xF;
        DMA_CNDTR(DMA1, DMA_CHANNEL_1)      = BUFFER_SIZE;
        DMA_CCR(DMA1, DMA_CHANNEL_1)        |= 1 << 0;
        ...
        DMA_CMAR(DMA1, DMA_CHANNEL_1)       = (uint32_t)(adc_buffer3 + 1); // memory = buffer
        DMA_IFCR(DMA1)                      = 0xF;
        DMA_CNDTR(DMA1, DMA_CHANNEL_1)      = BUFFER_SIZE;
        DMA_CCR(DMA1, DMA_CHANNEL_1)        |= 1 << 0;
        ...
        DMA_CMAR(DMA1, DMA_CHANNEL_1)       = (uint32_t)(adc_buffer4 + 1); // memory = buffer
        DMA_IFCR(DMA1)                      = 0xF;
        DMA_CNDTR(DMA1, DMA_CHANNEL_1)      = BUFFER_SIZE;
        DMA_CCR(DMA1, DMA_CHANNEL_1)        |= 1 << 0;
        ...
        while (DMA_CCR(DMA1,DMA_CHANNEL_7) & (1 << 0));
        ...
        DMA_CMAR(DMA1, DMA_CHANNEL_7)       = (uint32_t)adc_buffer2;
        DMA_CNDTR(DMA1, DMA_CHANNEL_7)      = ((BUFFER_SIZE + 1) * 2);
        DMA_CCR(DMA1, DMA_CHANNEL_7)        |= (1 << 0);
        delay_ms(DELAY_BUFFER_TRANSMIT);
        ...
        DMA_CMAR(DMA1, DMA_CHANNEL_7)       = (uint32_t)adc_buffer3;
        DMA_CNDTR(DMA1, DMA_CHANNEL_7)      = ((BUFFER_SIZE + 1) * 2);
        DMA_CCR(DMA1, DMA_CHANNEL_7)        |= (1 << 0);
        delay_ms(DELAY_BUFFER_TRANSMIT);
        ...
        DMA_CMAR(DMA1, DMA_CHANNEL_7)       = (uint32_t)adc_buffer4;
        DMA_CNDTR(DMA1, DMA_CHANNEL_7)      = ((BUFFER_SIZE + 1) * 2);
        DMA_CCR(DMA1, DMA_CHANNEL_7)        |= (1 << 0);
        delay_ms(DELAY_BUFFER_TRANSMIT);
...
void USART2_DMA_TX_Init(void)
{
    ...
    DMA_CCR(DMA1,DMA_CHANNEL_7)     |=  (1 << 4);   // DIR (memory -> peripheral)
    DMA_CCR(DMA1,DMA_CHANNEL_7)     |=  (0 << 5);   // PINC disabled
    DMA_CCR(DMA1,DMA_CHANNEL_7)     |=  (0 << 8);   // PSIZE = 8-bit
    DMA_CCR(DMA1,DMA_CHANNEL_7)     |=  (0 << 10);  // MSIZE = 8-bit
...
void ADC1_DMA_TIM2_Config(void)
{
	...
	//configure DMA
	DMA_CCR(DMA1,DMA_CHANNEL_1) &= ~(1<<0);
	DMA_CPAR(DMA1,DMA_CHANNEL_1) = (uint32_t)&ADC_DR(ADC1);      // peripheral = ADC data register
    DMA_CMAR(DMA1,DMA_CHANNEL_1) = (uint32_t)(adc_buffer1+1);    // memory = buffer
    DMA_CNDTR(DMA1,DMA_CHANNEL_1) = 1000;             			 // number of samples
    DMA_CCR(DMA1,DMA_CHANNEL_1) |= 1<<7;             			// MINC = memory increment
    DMA_CCR(DMA1,DMA_CHANNEL_1) |= 1<<8;              			// PSIZE = 16-bit
    DMA_CCR(DMA1,DMA_CHANNEL_1) |= 1<<10;             			// MSIZE = 16-bit
    DMA_CCR(DMA1,DMA_CHANNEL_1) &= ~(1<<4);           			// DIR = 0 (peripheral ? memory)
...
void TIM2_SlaveGateMode_TIM1(void)
{
	...
	TIM_CR2(TIM2)  &= ~(7 << 4);
	TIM_CR2(TIM2)  |=  (2 << 4);    // MMS = 010 ? Update event as TRG
    TIM_EGR(TIM2)   |= 1;
...
void TIM4_TriggerInit(void)
{
    ...
    TIM_ARR(TIM4) = DELAY_SILENT_ZONE;  
...
void RCC_Init(void)
{
    ...
    // flash speed is still less (depends on bus latency but not in cpu access)
    FLASH_ACR |= (4 << 0);  // LATENCY = 4 WS
    FLASH_ACR |= (1 << 8);  // PRFTEN  = prefetch enable
    FLASH_ACR |= (1 << 9);  // ICEN    = instruction cache enable
    FLASH_ACR |= (1 << 10); // DCEN    = data cache enable
    ...
    RCC_PLLCFGR |= (5  <<  4);  // PLLM   = 5 ? divider of /6 (PLLM+1 on STM32L4) 24/6=4
    RCC_PLLCFGR |= (40 <<  8);  // PLLN   = 40 4*40=160
    RCC_PLLCFGR |= (0  << 25);  // PLLR   = 0 ? divider of /2 160/2=80
    RCC_PLLCFGR |= (1  << 24);  // PLLREN = enable PLLR output
	...
	// Seperate pll setup for adc
	RCC_CR &= ~(1 << 26);              // PLLSAI1ON = 0

	while (RCC_CR & (1 << 27));        // wait PLLSAI1RDY = 0
	
    RCC_PLLSAI1CFGR |= (3  << 0);       // PLLSRC = HSE
	RCC_PLLSAI1CFGR	|= (5  << 4);       // PLLM = /6  ? 24/6 = 4 MHz input
	RCC_PLLSAI1CFGR	|= (40 << 8);       // PLLN = 24  ? 4*40 = 160 MHz VCO
	RCC_PLLSAI1CFGR	|= (0  << 25);      // PLLR = /2  ? 160/2 = 80 MHz ADC clock
	RCC_PLLSAI1CFGR	|= (1  << 24);      // PLLREN enable
